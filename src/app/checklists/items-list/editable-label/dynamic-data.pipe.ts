...
export class DynamicDataPipe implements PipeTransform {
  ...
  transform(value: string): string {
    ...
    while (match) {
      ...
      value = value.replace(DynamicDataPipe.GRT_FIELD, replaceWith);

      // Look for the next one.
      match = value.match(DynamicDataPipe.GRT_FIELD);
    }
    return value;
  }

  private _replacementFor(fieldNum: number): string | undefined {
