...
import { HEADER_COMMENT, LAST_UPDATED_FOOTER, METADATA_AIRCRAFT_TITLE, METADATA_CHECKLIST_TITLE, METADATA_COPYRIGHT_TITLE, METADATA_FILE_TITLE, METADATA_MAKE_MODEL_TITLE, METADATA_MANUFACTURER_TITLE, TextFormatOptions, WRAP_PREFIX } from "./text-format-options";
...
export class TextWriter {
        ...
        for (const item of items) {
            ...
            if (item.centered) {
                if (this._options.maxLineLength &&
                    fullLine.length < this._options.maxLineLength) {
    ...
    private _replaceNumbers(template: string, checklistNumber: number, itemNumber: number) {
        ...
        return template.replace('{{checklistNum}}', checklistNumber.toString()).replace('{{itemNum}}', itemNumber.toString());
    }
}