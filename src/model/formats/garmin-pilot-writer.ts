...
export class GarminPilotWriter {
  ...
  public async write(file: ChecklistFile): Promise<Blob> {
    return GarminPilotUtils.encrypt(
      await createTarGzip([
        {
          name: GarminPilotUtils.CONTENT_FILENAME,
          data: CryptoUtils.ENCODER.encode(
            GarminPilotChecklistContainer.toJsonString(this._checklistFileToGarmin(file), {
              enumAsInteger: true,
      ...
      .reduce<[GarminPilotChecklistItem, ChecklistItem][]>((accumulator, itemEFIS) => {
        ...
        switch (itemEFIS.type) {
          ...
          case ChecklistItem_Type.ITEM_CHALLENGE_RESPONSE: {
            const garminLiveDataItemType = GarminPilotUtils.getGarminLiveDataTypeByExpectation(itemEFIS.expectation);
